...
public class DeiUiMarqueeView extends View implements Runnable {
    ...
    public DeiUiMarqueeView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DeiUiMarqueeView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initattrs(AttributeSet attrs) {
        TintTypedArray tta = TintTypedArray.obtainStyledAttributes(getContext(), attrs,
                R.styleable.DeiUiMarqueeView);

        textColor = tta.getColor(R.styleable.DeiUiMarqueeView_mv_text_color, textColor);
        isClickStop = tta.getBoolean(R.styleable.DeiUiMarqueeView_mv_click_stop, isClickStop);
        isResetLocation = tta.getBoolean(R.styleable.DeiUiMarqueeView_mv_resetLocation, isResetLocation);
        speed = tta.getFloat(R.styleable.DeiUiMarqueeView_mv_text_speed, speed);
        textSize = tta.getFloat(R.styleable.DeiUiMarqueeView_mv_text_size, textSize);
        textDistance1 = tta.getInteger(R.styleable.DeiUiMarqueeView_mv_text_distance, textDistance1);
        startLocationDistance = tta.getFloat(R.styleable.DeiUiMarqueeView_mv_text_startLocation, startLocationDistance);
        repetType = tta.getInt(R.styleable.DeiUiMarqueeView_mv_repeat_type, repetType);
        tta.recycle();
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (resetInit) {
            ...
            xLocation = getPaddingLeft() + (getWidth() - getPaddingLeft() - getPaddingRight()) * startLocationDistance;
//            Log.e(TAG, "onMeasure: --- " + xLocation);
            resetInit = false;
        }
        if (contentWidth > (getWidth() - getPaddingLeft() - getPaddingRight())) {
            //需要判断滚动模式的
            switch (repetType) {
                case REPET_ONCETIME:
